...
import { bytesToHex, hexToBytes, toChecksumAddress } from './utils';
export class Wallet {
    /**
    ...
    signTypedData(domain, types, value) {
        ...
        const hash = hexToBytes(keccak256('\x19\x01' + domainHash.slice(2) + messageHash.slice(2)));
        const privateKeyBytes = hexToBytes(this.privateKey.slice(2));
        const { signature, recid } = secp256k1.ecdsaSign(hash, privateKeyBytes);
        const r = bytesToHex(signature.slice(0, 32));
        const s = bytesToHex(signature.slice(32, 64));
    ...
    signTransaction(tx) {
        ...
        return bytesToHex(signedEncoded);
    }
    /**
